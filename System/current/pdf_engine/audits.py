...
def structural_audit(root: Path, state: dict, require_materialized: bool=False, check_pdfs: bool=True) -> list[str]:
    ...
    if f.exists():
        txt=text_of_pdf(f)
        tokens=['1.1k.viii Profile Performance Audit & Advisory Improvement','1.1k.ix Audit Evidence, Efficiency & Governance','1.1a.iii Mutation Provenance & Causal Trace','1.1k.iv Selective Modularity & Mutation Blast-Radius Control','1.1k.v Theme-Authoring Workflow Profile & Approval-Gated Compilation','1.1k.x Reusable PDF Styler Extension & Preset Resolution','1.1k.xi Prompt-Agent Development & Epistemic Reasoning Audit','1.1k.xii Auditable Decision-Rationale Records & Evidence Linkage','1.1k.xiii Custom Prompt Profile & Capability Separation','1.1n.iv Strategic Plan, Capacity-Aware Segmentation & Checkpoint Continuity','1.1n.v Latency-Aware Execution Routing & Incremental Verification','1.1n.vi Release Impact Classification & Minimal Persistence','1.1o.iv Persistent Workspace Source-of-Truth & Export Demotion','1.1q Build, Render & Verify']
        for token in tokens:
            if token not in txt: issues.append(f'Formats missing required rule: {token}')
    if w.exists():
        txt=text_of_pdf(w)
        tokens=['2.7.9 Audit a Completed Profile Session','2.7.10 Diagnose Friction & Hand Off Proposals','2.7.11 Apply Reusable PDF Styler','2.7.12 Develop, Audit & Govern Prompt Agents','2.7.13 Record Material Agent Decisions','2.7.14 Use Custom Prompt Without Activating Capabilities','2.6.2 Run Regression & Mutation Simulation Preflight','2.6.3 Route Latency-Critical Execution','2.12.4 Maintain Strategic Plan, Segment & Checkpoint','2.13.4 Commit Persistent Workspace & Synchronize Stores','2.13.5 Classify Release Impact & Persist Minimally','2.15 Build, Render & Verify']
        for token in tokens:
