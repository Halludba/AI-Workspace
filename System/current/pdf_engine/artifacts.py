...
def create_ai_handoff(root: Path, state: dict) -> list[Path]:
    ...
    if runtime_cfg.get('pdf_styler_policy'):
        chunks += ['## PDF Styler', '', 'General PDF creation can use extension.pdf_styler with reusable named presets. Default: '+runtime_cfg['pdf_styler_policy']['default_style_id']+'. Explicit user/document/specialized-generator style authority outranks the default; fidelity must be disclosed when required fonts/assets are unavailable.', '']
    ...
    component_paths += [a['path'] for a in state.get('artifacts', []) if a.get('classification') == 'REQUIRED' and a.get('include_in_bundle', True) and str(a.get('path','')).endswith('.py')]
